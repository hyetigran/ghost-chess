...
- **Done**: guest identity already uses Supabase Anonymous Auth (`src/api/auth/index.ts`), matching `docs/adr/0005` — no separate guest-id scheme exists on this branch. `player_views` table, `redact_fen`/`sync_player_views` functions, the sync trigger, and its RLS policy are implemented (`supabase/schemas/06_functions.sql`, `07_player_views.sql`, migration `20260731220000_add_player_views.sql`), matching `docs/adr/0002`/`0003`. Client reads go through `player_views` (#12): `getGame`/`getActiveGames` read it directly (which also carries both player IDs, not secret, so the client can know who it's playing without touching `games`), and `games`/`moves` SELECT RLS denies direct reads while a game is `active` — the one status with real secrets. Clients also *subscribe* to `player_views` (#15), not just fetch it once: `useGameSubscription` opens a Realtime `postgres_changes` channel filtered by `game_id`, never subscribing to `games` (`supabase/schemas/08_realtime.sql` adds `player_views` to the `supabase_realtime` publication — `games` is deliberately not added). Server-side move validation (`docs/adr/0001`/`0007`) is implemented (#13): `supabase/functions/submit-move` is the only path that writes to `games`/`moves`, chess.js runs there (not client-side) against the true state, and the atomic `apply_move` RPC is locked to `service_role` only so it can't be called directly to bypass validation — `moves`' RLS has no INSERT policy at all now (denying every direct-client write by default), since #12's participant-scoped one existed only to unblock a client-side write path #13 has since fully replaced. Every illegal-move reason returns the identical response through the same code path. Both the redaction algorithm and the move-legality decision have independently unit-tested TypeScript mirrors (`src/lib/game/redact-fen.ts`, `src/lib/game/decide-move.ts`) — the actual enforcement stays in SQL/Deno, per each file's doc comment. Resignation (`endGame`) is separate from move submission and still goes through the `end_own_game` security-definer RPC added for #12 — `games`' SELECT RLS denying `active` rows blocks a direct client UPDATE from locating the row the same way it blocked `moves`' old INSERT path, and #13 doesn't touch resignation. Per-move deadline enforcement (`docs/adr/0006`, #14) is implemented: `gameSettingsSchema.timeControlHours` (1/12/24) replaced the old cumulative-clock `timeControl`/`timeIncrement` shape, `white_time_remaining`/`black_time_remaining` are gone from both `games` and `player_views` (nothing to persist — the deadline is derived from `updated_at` + `timeControlHours`, `src/lib/game/deadline.ts`), `submit-move` rejects a move once the mover's own deadline has lapsed, and a `pg_cron`-scheduled `forfeit_lapsed_games()` (every minute) forfeits any `active` game past its deadline with a new distinct `timeout` result value (see CONTEXT.md's Forfeit entry for why it's not folded into `abandoned`). The board component (#17) now takes a `redactedFen` prop instead of a generic `fen` — a naming-level guard against a future caller wiring in the true `games.fen` by mistake — actually uses `orientation` to flip the grid for black's perspective (previously accepted but ignored), highlights legal destinations for a selected piece, and restricts selection to the viewer's own pieces (PRD §5.1).
